import { getSocketRef, sendEvent } from "../socket";
import { useGameStore, usePlayerInfo } from "../store";

export const PlayOrPassModal: React.FC = () => {
  const state = useGameStore((state) => state.faceOff);
  const { player } = usePlayerInfo();
  if (state?.stage !== "PLAY_OR_PASS") return null;
  ...
  return (
    <div className="fixed left-0 right-0 top-0 bottom-0 flex bg-opacity-60 bg-black z-[10000] items-center justify-center">
      <div className="bg-white p-4 min-w-[]">
        <h2 className="p-2 text-center">Play or Pass?</h2>
        <div className="flex justify-evenly gap-2">
          <button
            onClick={() => {
              sendEvent({ event: "playOrPass", play: true });
            }}
            type="button"
          ...
          <button
            onClick={() => {
              sendEvent({ event: "playOrPass", play: false });
            }}
            type="button"
